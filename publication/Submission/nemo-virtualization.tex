...
Due to the wall-time limitation at \NEMO, \Roced is going to drain virtual machines before hitting the limit. Draining job slots do not accept new jobs, while already started once potentially still occupy parts of the nodes in draining mode, which can lead to an overall efficiency degradation.  One counter measure is to utilze those nodes in a so called backfilling mode by taking into account the remaining wall-time and to assign preferable short running jobs to the concerned nodes.
